...
class GAM_Updater {
	...
	public function handle_errors( $errors ) {
		if ( ! empty( $errors['no_key'] ) ) {
			$this->add_error( sprintf( __('A licence key for %s could not be found. Maybe you forgot to enter a licence key when setting up %s.', 'wp-event-manager-calendar'), esc_html( $this->plugin_data['Name'] ), esc_html( $this->plugin_data['Name'] ) ) );
